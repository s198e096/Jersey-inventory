...
def get_sale_by_id(sale_id):
    with get_conn() as conn:
        row = conn.execute("""
            SELECT s.*, i.team, i.player_name, i.jersey_number, i.version, i.size, i.unit_cost
            FROM sales s
            JOIN inventory i ON i.id = s.inventory_id
            WHERE s.id = ?
        """, (int(sale_id),)).fetchone()
    return row


def update_sale(sale_id, quantity, sale_price_each, fees, shipping_cost, notes):
    """
    Update a sale while keeping stock valid for the inventory lot tied to that sale.
    """
    with get_conn() as conn:
        sale = conn.execute(
        ...
        inventory_id = int(sale["inventory_id"])
        old_qty = int(sale["quantity"])

        inv_row = conn.execute(
            "SELECT quantity_received FROM inventory WHERE id = ?",
            (inventory_id,)
        ).fetchone()
        if inv_row is None:
            raise ValueError("Linked inventory lot not found.")

        other_sold = conn.execute(
            "SELECT COALESCE(SUM(quantity), 0) FROM sales WHERE inventory_id = ? AND id != ?",
            (inventory_id, int(sale_id))
        ).fetchone()[0]

        max_allowed = int(inv_row["quantity_received"]) - int(other_sold)
        if int(quantity) > max_allowed:
            raise ValueError(
                f"This inventory lot can support at most {max_allowed} unit(s) for this sale."
            )
